...
export async function createSpec(run: RunRecord): Promise<SpecContract> {
  ...
  const spec: SpecContract = {
    ...
    // the acceptance criteria. renderContract leads with these in /goal format.
    // When the task is already a /goal-formatted planner prompt, recover its
    // explicit objective + done-when instead of re-deriving (no nested /goal).
...
export function renderContract(spec: SpecContract): string {
  ...
  const body = [
    ...
    .filter(Boolean)
    .join("\n");
  // Lead with the /goal-format header so the backend (Claude or Codex) picks up
  // the objective and the verifiable stopping condition before the body.
