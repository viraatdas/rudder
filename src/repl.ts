...
export async function runInteractiveShell(defaults?: {
  ...
  try {
    while (true) {
      const line = await rl.question(`${promptLabel(state)} `).catch((error: unknown) => {
...
async function handleSlashCommand(line: string, state: ShellState): Promise<boolean> {
...
async function runTaskFromShell(task: string, state: ShellState): Promise<void> {
...
async function watchFromShell(runId?: string): Promise<void> {
  const controller = new AbortController();
  const onSigint = () => {
    controller.abort();
...
function promptLabel(state: ShellState): string {
  void state;
  return ">";
}

function formatBackend(state: ShellState): string {
  return `${state.backend}${state.model ? ` (${state.model})` : ""}`;
}

function modelForBackend(backend: BackendId, config: Awaited<ReturnType<typeof loadConfig>>): string | undefined {
  if (backend === "claude") {
