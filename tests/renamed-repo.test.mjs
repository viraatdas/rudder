...
// save while worktree.path still said the old name, so healing cannot key on
...
test("heals both-stale records via the stored-root prefix", () => {
  const record = {
    ...
    worktree: { path: "/old/place/aws-v2/.rudder-worktrees/aws-v2-abc/task-def" },
  ...
  assert.equal(
    ...
    "/new/place/libra/.rudder-worktrees/aws-v2-abc/task-def",
  );
});
...
test("heals the field-observed shape: repoRoot new, nested paths stale", () => {
  ...
  assert.equal(
    record.worktree.path,
    "/new/place/libra/.rudder-worktrees/aws-v2-abc/task-def",
    "the .rudder-worktrees marker rebases even when the stored root is useless as a key",
  );
});
...
test("a record that is already true is left byte-identical", () => {
  const record = {
    id: "r3",
    repoRoot: "/new/place/libra",
    worktree: { path: "/new/place/libra/.rudder-worktrees/libra-abc/task-def" },
  };
