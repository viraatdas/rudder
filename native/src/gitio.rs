...
pub(crate) fn write_rudder_context_with_history(
    ...
    append_ship_status(&mut body, repo_root);
    ...
    let ready_agents = agents
        .iter()
        .filter(|agent| agent.status == AgentStatus::Done)
        .collect::<Vec<_>>();
...
fn append_ship_status(body: &mut String, repo_root: &Path) {
    let branch = current_branch_at(repo_root).unwrap_or_else(|| "HEAD".to_string());
    ...
    match compare_ref {
        None => {
            ...
            body.push_str(
                "- deployed: whatever your host serves CANNOT include any of this work until you add a remote and `git push`.\n",
            );
        }
        Some(remote) => {
            let ahead = git_output_args(repo_root, &["rev-list", "--count", &format!("{remote}..HEAD")])
                .ok()
                .and_then(|value| value.trim().parse::<usize>().ok())
                .unwrap_or(0);
            if ahead == 0 {
                body.push_str(&format!("- pushed: YES — {branch} is up to date with {remote}.\n"));
                body.push_str(&format!(
                    "- deployed: {remote} has all local commits; whether it is live depends on your host/CI deploying from {remote}.\n"
                ));
            } else {
                body.push_str(&format!(
                ...
                body.push_str(&format!(
                    "- deployed: whatever deploys from {remote} does NOT include these {ahead} commit(s). Run `git push` to ship them.\n"
                ));
            }
        }
    }
