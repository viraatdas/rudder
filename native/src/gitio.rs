...
pub(crate) fn write_rudder_context(
    ...
    let body = merge_generated_rudder_md(
        &fs::read_to_string(repo_root.join("RUDDER.md")).unwrap_or_default(),
        &generated,
    );
...
pub(crate) fn merge_generated_rudder_md(existing: &str, generated: &str) -> String {
    let wrapped = format!("{RUDDER_GENERATED_START}\n{generated}\n{RUDDER_GENERATED_END}\n");
    if let Some(start) = existing.find(RUDDER_GENERATED_START) {
        if let Some(end_rel) = existing[start..].find(RUDDER_GENERATED_END) {
            let end = start + end_rel + RUDDER_GENERATED_END.len();
    ...
    if let Some(plan) = latest_rudder_plan_block_for_context(existing) {
...
fn latest_rudder_plan_block_for_context(text: &str) -> Option<String> {
