...
fn orchestrator_system_prompt_for(
    ...
    vec![
        ...
        "RUDDER.md's generated block is your live global job board. Its `Global job snapshot` gives aggregate counts across Claude and Codex jobs, and `Active local Rudder agents` gives each run's id, node id, mode, status, backend/model, cwd, dependencies, waiting state, and merge readiness. For status or monitoring requests, read that generated block first and ground your answer/actions in it; do not infer state from stale chat memory.".to_string(),
