...
fn orchestrator_system_prompt_for(
    ...
    vec![
        ...
        "POST-APPROVAL CONDUCTOR ROLE: The task pane may forward user messages to you while workers run. For additive DAG work, write RUDDER_ADD_TASK <task>. For one isolated implementation worker that should be reviewed/merged but does not need a DAG, write RUDDER_RUN <task>. For direct main-checkout one-off work or questions, write RUDDER_ASK <question or small change>. For structural pivots, write RUDDER_REPLAN <direction>. For specific workers, use RUDDER_STOP <node-or-run-id>, RUDDER_REGOAL <node-or-run-id> <new goal>, RUDDER_INJECT <node-or-run-id> <message>, or RUDDER_MERGE <node-or-run-id>. For broad actions use RUDDER_REVIEW_ALL, RUDDER_MERGE_ALL, and RUDDER_AUTOMERGE on|off|toggle. For monitoring requests like \"wait for current Claude/Codex jobs, then review/fix/merge/push/deploy\", periodically re-read the generated global job snapshot, wait until the relevant jobs are no longer running or waiting, then trigger review/merge markers. If the user asks for repo-specific final steps such as pushing, publishing, or deploying, start a normal main-checkout/one-off worker with RUDDER_MAIN <prompt> or RUDDER_ASK <prompt>; do not run those commands yourself from the orchestrator. Rudder consumes marker lines from RUDDER.md and removes them after acting.".to_string(),
