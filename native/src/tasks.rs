...
fn orchestrator_system_prompt_for(
    ...
    vec![
        ...
        "RUDDER.md's generated block is your live global job board. Its `Global job snapshot` gives aggregate counts across Claude and Codex jobs, `Active local Rudder agents` lists only live or waiting runs, `Ready local Rudder agents` lists completed work awaiting review or merge, and `Completed local Rudder agents` lists terminal history. For status or monitoring requests, read that generated block first and ground your answer/actions in it; do not infer state from stale chat memory.".to_string(),
        ...
        "POST-APPROVAL CONDUCTOR ROLE: The task pane may forward user messages to you while workers run. For additive DAG work, write RUDDER_ADD_TASK <task>. For one isolated implementation worker that should be reviewed/merged but does not need a DAG, write RUDDER_RUN <task>. For direct main-checkout one-off work or questions, write RUDDER_ASK <question or small change>. For structural pivots, write RUDDER_REPLAN <direction>. For specific workers, use RUDDER_STOP <node-or-run-id>, RUDDER_RESUME <node-or-run-id> <claude|codex> <model> [effort] [new direction], RUDDER_REGOAL <node-or-run-id> <new goal>, RUDDER_INJECT <node-or-run-id> <message>, or RUDDER_MERGE <node-or-run-id>. Resolve natural-language references such as 'current tasks' from the live rows in RUDDER.md; for 'pause and resume on model X', write STOP then RESUME for each matching worker. When the user asks to take/move/run the current or active agent fleet on Rudder Cloud, write RUDDER_CLOUD_MIGRATE; never use the single-run onload action for a fleet. Planned DAG nodes integrate automatically; for broad actions use RUDDER_REVIEW_ALL and RUDDER_MERGE_ALL. For monitoring requests like \"wait for current Claude/Codex jobs, then review/fix/merge/push/deploy\", periodically re-read the generated global job snapshot, wait until the relevant jobs are no longer running or waiting, then trigger review/merge markers. If the user asks for repo-specific final steps such as pushing, publishing, or deploying, start a normal main-checkout/one-off worker with RUDDER_MAIN <prompt> or RUDDER_ASK <prompt>; do not run those commands yourself from the orchestrator. AUTO-SHIP: if the ORIGINAL task goal includes deploying/publishing/shipping to a host (e.g. 'deploy to X', 'ship it', 'make it live'), treat shipping as the plan's final step — do NOT wait for the user to ask again. Once every implementation node has merged (merged count = launched, none running/waiting) and the Ship status shows unpushed work, automatically write RUDDER_MAIN <push the integrated result with `jj git push` (or `git push`), then deploy to the named target — or rely on the host auto-deploying from the push, e.g. Netlify/Vercel>. After it finishes, verify the live target before telling the user it is deployed; 'merged' is local git only and is never deployed until pushed. Rudder consumes marker lines from RUDDER.md and removes them after acting.".to_string(),
...
pub(crate) fn rudder_orchestrator_prompt(task: &str) -> String {
    let task = strip_rudder_prompt_wrappers(task);
    let base = "You are Rudder's orchestrator for this checkout. Read CLAUDE.md and AGENTS.md first if they exist (the repo's rules bind you and the plans you write). Read RUDDER.md if it exists, and read RUDDER_SHARED.md if it exists. RUDDER.md's generated `Global job snapshot`, `Active local Rudder agents`, `Ready local Rudder agents`, and `Completed local Rudder agents` sections are your live view of current Claude/Codex/Rudder jobs: use them for status and monitoring requests instead of guessing from memory. Treat Active as live or waiting only, Ready as completed work awaiting review or merge, and Completed as terminal history. RUDDER_SHARED.md is gitignored local context for all agents; if the user shares tokens, credentials, private URLs, account ids, or env details in this conversation, append them there so workers can read them later. If the user asks to improve agent behavior, project instructions, or context quality, run `rudder context audit --json` and use its findings as evidence before planning cleanup work. If no concrete task is given yet, say you are ready and wait. For a concrete request, ask clarifying questions when needed; once clear, write the DAG to RUDDER.md outside Rudder's generated block and ask the user to approve. Only AFTER they explicitly confirm, add a `RUDDER_APPROVE_PLAN` line to RUDDER.md (keeping the plan block) to launch. After launch, stay alive as the high-level conductor: answer status questions from the generated global snapshot and control workers through RUDDER_* markers, never by editing product files yourself.";
    ...
    pub(crate) fn to_task(&self) -> RudderPlanTask {
        let mut deps: Vec<PlanEdge> = self
            ...
            .map(|on| PlanEdge {
                on: on.clone(),
                edge: EdgeType::Hard,
                // A PlannedNode only reaches `deps` once its hard edge was
                // accepted (LLM-justified at parse time, or user-typed in plan
                // review). Re-serializing with why:None let the parser's
                // "unjustified hard → soft" demotion silently erase the
                // ordering on every RUDDER.md round-trip.
                why: Some("recorded hard dependency".to_string()),
            })
            .collect();
...
pub(crate) fn extract_rudder_plan_tasks_with_frontier(
    ...
    let mut out = Vec::new();
    // Ids emitted so far. Downstream readiness/merge/scheduling all match nodes by
    // id STRING, so two tasks sharing an id would let one satisfy a dependency the
    // other owns and silently drop the second node's work. Uniquify later collisions
    // here (the reconcile/rebase path already does this via uniquify_node_id).
    ...
    for (index, task) in capped.iter().enumerate() {
        ...
        let mut id = plan_task_id(task, index);
        if !emitted_ids.insert(id.clone()) {
            // Collision with an id already emitted this parse: rename the later
            // duplicate so it stays a distinct node. Cross-references to the shared
            // id continue to resolve to the FIRST occurrence (a malformed plan, but
            // never silent work loss).
            let mut n = 2;
            let renamed = loop {
...
pub(crate) fn diff_plan(
    ...
    for task in new_tasks {
        ...
        // Skip nodes already claimed by an earlier task this pass, or two new tasks with
        // the same title would both bind one running node — wrongly stopping a distinct
        // node and silently losing a planned task.
...
/// completion-note shape, tolerant of surrounding prose/markdown.
